...
1. Exactly 197 unique `test.json` leaves exist across exactly 19 planned categories, each bijectively and individually listed in `coverage-tracking.md` and exactly once in `WORKITEMS.md`.
...
7. Raw↔SDK four-way interoperability and replica independence are expressed without provider-specific logic except explicitly allowed metadata normalization.
...
12. Validation fails closed on malformed JSON, missing files/fields, duplicate IDs, coverage/WORKITEM drift, unknown assertions, empty architecture globs, leaked wrapper tags, stale ADR citations, and prohibited golden auto-update behavior.
