...
for fragment in ('## Policy requirement notation','unknown IDs, selectors, operators, or unresolved paths fail closed','`;` joins predicates with logical AND','`overwrite[...]` additionally requires replacement','predicates sharing that prefix must be satisfied by the same rate rule','selector suffixed `_subset` instead requires the resolved set to be contained','A bare selector with no operator names a boolean artifact property','`[]` is the empty-list literal','A `correlation_obligation` is not prose evidence'):
